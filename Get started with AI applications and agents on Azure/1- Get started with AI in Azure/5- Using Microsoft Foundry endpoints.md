...
![Screenshot of the model endpoint and key in the Foundry Playground's details page](asset/5- Using Microsoft Foundry endpoints-1-model-endpoint-key.png)
...
The results of the request are returned as a response, also with headers and a body. For example, the response might include the reply generated by a model from the prompt. The response comes back in JSON format. A section of that JSON may look like the following:
