...
*Training PDF (browser export):* [5- Get started with speech in Microsoft Foundry _ Get started with AI apps and agents in Azure.pdf](./5- Get started with speech in Microsoft Foundry _ Get started with AI apps and agents in Azure.pdf)
...
3. Select **Create**. Wait for your project to be created. After creating or selecting a project in the new Foundry portal, it should open in a page similar to the following image:
...
2. In the model drop-down list, ensure that a **gpt-4.1** model has been deployed and selected for your agent.

3. Assign your agent the following **Instructions**:
...
5. Test the agent by entering the following prompt in the **Chat** pane:
...
![speech-agent in the agent playground (model, instructions, chat).](asset/5- Exercise-02-speech-agent-playground.png)
...
2. In the Configuration pane, under **Voice Live**, review the default speech input and output configuration. You can try different voices, previewing them until you decide which one to use.

3. Close the Configuration pane and use the **Save** button to save the agent.
...
![Voice session: Listening status in the chat pane.](asset/5- Exercise-03-speech-agent-voice-listening.png)

2. When the app status is **Listening…**, say something like "How does speech recognition work?" and wait for a response.

3. Verify that the app status changes to **Processing….** The app will process the spoken input.

> **Tip:** The processing speed may be so fast that you do not actually see the status before it changes back to **Speaking**.

4. When the status changes to **Speaking…**, the app uses text-to-speech to vocalize the response from the model. To see the original prompt and the response as text, select the **cc** button on the bottom of the chat screen.
...
5. To continue the conversation, just ask another question, such as "How does speech synthesis work?", and review the response.

6. When you have finished chatting with the agent, use the **X** icon to end the session. A transcript of the conversation will be displayed.
...
2. Review the code; noting that it handles:
