...
*Training PDF (browser export):* [3- Speech synthesis - Training _ Microsoft Learn.pdf](./3- Speech synthesis - Training _ Microsoft Learn.pdf)
...
![Microsoft Foundry: Azure Speech - Text to Speech playground (Preview tab, sample text and voice controls).](./asset/3- Speech synthesis-1-foundry-text-to-speech-playground.png)
...
![Visual Studio Code: `text-message.txt` open with the message text to synthesize.](./asset/3- Speech synthesis-2-vscode-text-message-txt.png)

First, connect to the endpoint for Azure Speech. Then, create a `SpeechSynthesizer` object. Then the application processes the text file containing the message and uses the `SpeechSynthesizer` object to generate the spoken audio.
...
![Visual Studio Code: `text-to-speech.py` sample that reads `text-message.txt` and synthesizes speech.](./asset/3- Speech synthesis-3-vscode-text-to-speech-py.png)

When you run the application, it will take the text and return an audio output of the message.

![Visual Studio Code terminal: `python text-to-speech.py` output (`Synthesizing speech...` and the message text).](./asset/3- Speech synthesis-4-terminal-speech-synthesis-output.png)

Next, learn how to incorporate speech-to-speech capabilities into an application with Azure Speech - Voice Live.

*Next unit: Creating a speech-capable agent*
