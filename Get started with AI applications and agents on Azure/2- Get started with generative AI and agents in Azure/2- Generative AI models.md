...
![Screenshot of Foundry's model catalog with the new UI.](asset/2- Generative AI models-1-model-catalog.png)
...
- Model descriptions and capabilities (text generation, reasoning, coding, multimodal, embeddings, and more)
...
![Screenshot of Foundry's model entries with GPT-4.1 as an example.](asset/2- Generative AI models-2-gpt-4-1-details.png)
...
- Mistral Large 3 (Mistral AI): A state-of-the-art, general-purpose model ideal for scenarios where you want strong quality with efficient throughput. The model does well with multilingual drafting, structured business report generation, or mid-latency agent tasks that balance cost and performance.
...
- Model leaderboards: Leaderboards rank models based on attributes like quality, safety, and throughput. This helps identify the best model for a task. Examples include reasoning, summarization, and code generation.
...
![Screenshot of Foundry's model leaderboard and side-by-side comparisons.](asset/2- Generative AI models-3-model-leaderboard.png)
...
Deployment-level quotas define how many tokens or requests can be processed before throttling occurs. Larger prompts and higher max output token settings consume more TPM, leading to rate-limit errors if exceeded. If you see throttling, lower max tokens or reduce concurrent requests in code.
