...
![Screenshot of the Foundry project home page](asset/5- Excercise-01-foundry-project-home.png)
...
![Screenshot of the AI Foundry model catalog](asset/5- Excercise-02-model-catalog.png)
...
![Screenshot of the gpt-4.1-mini model page](asset/5- Excercise-03-gpt-4-1-mini-model.png)
...
![Screenshot of the model playground](asset/5- Excercise-04-model-playground.png)
...
In the Chat pane, enter a prompt such as Who was Ada Lovelace?, and review the response.

![Screenshot of the chat pane with a response](asset/5- Excercise-05-chat-with-response.png)

Enter a follow-up prompt, such as Tell me more about her work with Charles Babbage. and review the response.
...
Enter a new prompt, such as Tell me about the ELIZA chatbot. and view the response.
Continue the conversation with prompts such as How does it compare with modern LLMs?.
...
In the Chat pane, view the Code tab. This tab shows sample code that a client application can use to chat with the model. Above the sample code, you can choose preferences for:
...
Microsoft Entra ID authentication: The client app presents an authentication token based on an identity that is assigned to it (or to the current user).
...
In the model playground, switch back to the Chat tab. Then, at the top-right of the chat pane, use the New chat button to restart the conversation and removes the conversation history.
...
Now enter a new user prompt related to expense claims, such as What kinds of business expense are typically reimbursed by employers?

Review the response, which should provide some general guidance about expense claims.

Try re-asking a previously-asked question that is unrelated to expenses, such as Tell me about the ELIZA chatbot; and compare the response now that the system prompt has changed.
...
In the model playground, at the top right select Save as agent. Then, when prompted, name your new agent expenses-agent.

When the agent is created, it opens in a new playground specifically for working with agents.

![Screenshot of the agent playground](asset/5- Excercise-06-agent-playground.png)
...
code
 ...
 definition:
   ...
   tools: []
 status: active
Switch back to the Chat tab, and enter the prompt Who are you?

The response should indicate that the agent is “aware” of its role as an expense claims advisor.

Enter an expenses-related prompt, such as How much can I claim for a taxi?
...
Open a new browser tab, and view the expenses_policy.docx at https://microsoftlearning.github.io/mslearn-ai-fundamentals/data/expenses_policy.docx. We’ll use this to provide a knowledge source that the agent can use to answer questions about expense claims.
Download expenses_policy.docx to your local computer.
Return to the tab containing the agent playground, and in the pane on the left, expand the Tools section if it’s not already expanded.
Upload the expenses_policy.docx file, creating a new index with the default index name. When the index has been created, attach it to the agent.
At the top of the agent playground, use the Save button to update the agent definition.
In the pane on the right, view the YAML tab, which contains the definition for your agent. Note that its definition now includes the file search tool you added (in the tools section):
...
code
 ...
 status: active
Switch back to the Chat tab, and enter the same expenses-related prompt as before (for example, How much can I claim for a taxi?) and view the response.

This time the response should be informed by the information in the expenses data source.

Try a few more expenses-related prompts, like What about a hotel? or Can I claim the cost of my dinner?
...
In the agent playground, switch from the Chat tab to the Code tab, and view the sample code for consuming the agent; which should be similar to this:
...
The code to connect to your agent uses the Azure.AI.Projects library to create an AIProjectClient object connected to your Foundry project. Since this involves connecting to a project, which may contain privileged resources, key-based authentication is not supported, and the application must use an Entra ID identity to be authenticated.
...
In the Code tab, use the Open in VS Code for the web button to open Visual Studio Code for the Web in a new browser tab.

Wait for the environment to be set up.

Tip: It can take a few minutes to set the environment up!

![Screenshot of VS Code for the Web](asset/5- Excercise-07-vscode-web.png)

After VS Code for the web has opened and the environment has been set up, close the GitHub Copilot Chat pane on the right side to give you more room, and note that the Instructions.md file contains the instructions you need to run the sample code (which is in the run_agent.py file in the VS Code Explorer pane on the left.)
In the terminal pane at the bottom, enter the following command to run the code:

code
python run_agent.py
The output should include a response to the prompt Tell me what you can help with.

![Screenshot of VS Code for the Web with output from the agent](asset/5- Excercise-08-vscode-agent-output.png)

Tip: If an authentication issue occurs, you may need to sign into Azure in the VS Code terminal by using the Azure CLI az login command. See the Azure CLI documentation for details.
...
Keep the VS Code for the Web tab open, but switch back to the Foundry portal tab.
In the agent playground, in the Publish drop-down list, select Publish agent.
...
![Screenshot of the agent publishing confirmation message](asset/5- Excercise-09-agent-published.png)

Note that you can perform additional steps to publish your agent for integration with Teams and Microsoft 365 Copilot. However, in this exercise, select Close.

Tip: You can use the View details option in the Publish drop-down list to re-open the agent details.

Switch back to the VS Code for the Web tab, and in the Explorer pane, add a new file named expenses-client.py.
Add the following code to the new expenses-client.py file.
...
Replace the YOUR_AGENT_ENDPOINT placeholder with the Responses API endpoint for your agent (copied from the published agent details in the Foundry portal).
Save the changes to the expenses-client.py code file (CTRL+S).
In the VS Code terminal pane, enter the following command to run the code.
...
![Screenshot of the expenses-client.py output in VS Code for the Web](asset/5- Excercise-10-expenses-client-output.png)
...
Tip: If you want to keep the Foundry project, but avoid being charged for the published agent, use the ⋮ menu next to the Publish drop-down list to delete the agent.