...
![Diagram of a filter: the kernel slides across the image, computing a new value for each position (convolution).](./filter.gif)
...
| --- | --- |
| ![Grayscale photograph of a dog holding a stick in its mouth.](../assets/cv-image-processing-dog-original.png) | The same photograph with a Laplace filter applied, emphasizing edges of the dog, the stick, and other sharp intensity changes. |
