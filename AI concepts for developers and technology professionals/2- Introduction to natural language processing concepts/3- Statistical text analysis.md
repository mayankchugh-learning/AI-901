...
![Module overview: statistical text analysis and frequency analysis](../assets/nlp-statistical-text-analysis-01-frequency-intro.png)
...
![Extended term-frequency table, summary line, and start of TF‑IDF with Sample A and Sample B](../assets/nlp-statistical-text-analysis-02-frequency-tfidf-samples.png)
...
![Top term frequencies for Sample A and Sample B](../assets/nlp-statistical-text-analysis-03-term-frequency-tables.png)
...
![TF‑IDF calculation steps and Sample B term table](../assets/nlp-statistical-text-analysis-04-tfidf-calculation.png)
...
![TF‑IDF scores for Sample A and Sample B, and start of bag-of-words section](../assets/nlp-statistical-text-analysis-05-tfidf-bag-of-words.png)
...
![Bag-of-words definition, examples, and introduction to TextRank](../assets/nlp-statistical-text-analysis-06-bag-of-words-textrank.png)

**Bag-of-words** is the name given to a **feature extraction technique** that represents text tokens as a **vector of word frequencies or occurrences**, deliberately **ignoring grammar and word order**. This representation becomes the input for machine learning algorithms like **Naive Bayes**, a probabilistic classifier that applies Bayes’ theorem to predict the probable class of a document based on word frequency.
...
You can implement **sentiment analysis** by using the same method to classify text by emotional tone. The bag-of-words provides the features, and the model uses those features to estimate probabilities and assign sentiment labels like `"positive"` or `"negative"`.

## TextRank

**TextRank** is an unsupervised graph-based algorithm that models text as a network of connected **nodes**. For example, each sentence in a document could be considered a node, and the connections (**edges**) between them are scored based on the similarity of the words they contain. TextRank is commonly used for **text summarization** by identifying a subset of sentences that best represent the overall subject.

The TextRank algorithm applies the same principle as **Google's PageRank algorithm** (which ranks web pages based on links between them) to text. The key idea is that a sentence is *important* if it is similar to many other *important* sentences. The algorithm works through the following steps:
...
2. **Calculate ranks iteratively:** Each node's score is calculated based on the scores of the nodes connected to it. The formula is:

   `TextRank(S_i) = (1-d) + d * Σ(w_ji / Σw_jk) * TextRank(S_j)`

   where `d` is a damping factor (typically `0.85`); `w_ji` is the weight of the edge from sentence `j` to sentence `i`; and the sum iterates over all sentences connected to `i`.

3. **Extract top-ranked sentences:** After the calculation converges, the sentences with the highest scores are selected for the summary.
...
![TextRank: sentence split, similarity weights, and sentence graph](../assets/nlp-statistical-text-analysis-07-textrank-graph.png)
...
*Diagram of connected sentence nodes.*

![TextRank graph, resulting summary, Note on extractive vs abstractive summarization, and word-level keyword extraction](../assets/nlp-statistical-text-analysis-08-textrank-summary-note.png)

After calculating TextRank scores iteratively using these weights, sentences 1, 3, and 5 might receive the highest scores because they connect well to other sentences. These sentences would be selected to form a concise summary:
...
> **Note**
>
> **Extractive summarization:** Selecting a subset of the original text (as shown in the TextRank example).
>
> **Abstractive summarization:** Generating new language to summarize key themes using semantic modeling.
